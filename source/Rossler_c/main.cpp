...
int main( int argc, char** argv )
{
    ...
    const double c_max = 8.0;
    int m = 9001;
    double dc = (c_max - c_min) / (m - 1);

    std::ofstream  data_file( "Rossler_c.d" ); // файл с числ. данными;
    ...
    for ( size_t k = 0;  k < m; ++k  )
    {
        ...
        for ( size_t i = 1; i < n - 1;  ++i )
        {
            ...
            if ( ( p_l.x < p_c.x ) && (p_c.x > p_r.x ) )
                data_file << std::setw(16) << c << std::setw(16) << p_c.x << "\n";
            p_l = p_c;
            p_c = p_r;
        }
        c += dc;
    }
    std::cout << "Data file: Rossler_c.d\n";
    data_file.close();
    return 0;
}
